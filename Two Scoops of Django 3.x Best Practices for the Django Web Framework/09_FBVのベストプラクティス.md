...
### 9.1 FBVの利点
- FBVのシンプルさは、コードの再利用性を犠牲にしている。FBVには、スーパークラスを継承する機能がない。
- FBVはより関数的な性質を持っている。
...
- すべての関数はHttpRequestオブジェクトを受け取り、HttpResponseオブジェクトを返すということを覚えておけば、使うことができる。
